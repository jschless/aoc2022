...
fn day15() {
    ...
    for x in 0..4000000 {
        ...
        for c in &coverage[1..] {
            if low < 0 {
                low = 0;
            } else if high > 4000000 {
                high = 4000000;
            } else if c.0 < high && c.1 > high && c.0 >= low {
        ...
        if condensed_coverage.iter().map(|t| t.1 - t.0).sum::<i32>() < 4000000 {
            println!("PROBLEM CHILD: P{}", x);
        }
        condensed_coverage.push((low, high));
        ...
        //     condensed_coverage
        //         .iter()
        //         .map(|t| (min(t.1, 4000000) - max(t.0, 0)))
        //         .sum::<i32>()
        // );
    }
    for b in beac {
